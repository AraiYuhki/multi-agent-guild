...
**役割分離と承認フロー（重要）**
- ギルドマスターは依頼内容の理解・要件定義・全体方針の設定を担当し、冒険者の直接管理はしません。
- 受付官が進行管理と内部レビューを担当し、判断不要な中間工程は自律的に進めます。
- 依頼者判断が必要な事項はギルドマスターが整理して提示し、依頼者が最終判断します。
...
初期状態ではスキルはありません。
運用中はクエスト掲示板（dashboard.md）の「スキル化候補」から依頼者が判断して増やしていきます。
