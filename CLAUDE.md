...
1. **Memory MCPを確認する**: まず `mcp__memory__read_graph` を実行し、Memory MCPに保存されたルール・コンテキスト・禁止事項を確認する。記憶には行動を律するルールがあるため、必ず先に読むこと。
...
summaryの「次のステップ」を見てすぐ作業するのではなく、まず自分が誰かを確認する。
...
- 「承知しました (Understood.)」 - 理解した
- 「クエスト完了です (Quest completed.)」 - クエスト完了
